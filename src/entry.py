    ...
    async def fetch(self, request):
        ...
        try:
            ...
            if path == "/callback":
                ...
                # save tokens in KV (optional)
                try:
                    if getattr(self.env, "OAUTH_TOKENS", None):
                        await self.env.OAUTH_TOKENS.put(f"user:{user['id']}", json.dumps(token_json))
                except Exception as e:
                ...
                # формируем заголовки: редирект + Set-Cookie (явно указываем Domain)
                redirect_to = FRONTEND_URL + "#token=" + urllib.parse.quote(jwt, safe='')
                cookie_value = (
                    f"session={jwt}; Domain=pollpi.slavi.workers.dev; Path=/; "
                    f"HttpOnly; Secure; SameSite=None; Max-Age=86400"
                )
